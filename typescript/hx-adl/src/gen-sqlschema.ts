...
export function configureCli(program: Command) {
  program
    ...
    .option('--as-alter-statements', 'Generate sql in alter format', false)
    ...
    .action((adlFiles: string[], cmd: {}) => {
      ...
      generateSql({ adlFiles, adlSearchPath, outfile, outfiles, outmetadata, extensions, templates, dbProfile });
    });
}
...
async function generateSqlSchema(params: Params, loadedAdl: LoadedAdl, dbTables: DbTable[]): Promise<void> {
  ...
  let writer = fs.createWriteStream(params.outfile);
