...
import { Annotations, Module, ScopedDecl, TypeExpr } from '../adl-gen/runtime/sys/adlast';
...
function addCode(importingHelper: ImportingHelper, loadedAdl: LoadedAdl, codeGenType: CodeGenType, codeGen: CodeGen, typeExpr: TypeExpr, name: string, comment: string | null, apiDefaults: sys_types.Maybe<{}|null>) {
  ...
  if (typeExpr.typeRef.value.name === "HttpPost") {
    ...
    switch (codeGenType) {
      ...
      case "decl": {
        ...
        codeGen.add(
          `${importingHelper.asReferencedName(responseType)} ${camelCase(name)}(HxContext ctx, ${importingHelper.asReferencedName(requestType)} req);`
        );
        codeGen.add("");
        return;
      }
      case "ctor": {
        ...
        codeGen.add(`// ${JSON.stringify(apiDefaults)}`);
        codeGen.add("");
        return;
      }
      case "impl": {
  ...
  if (typeExpr.typeRef.value.name === "HttpGet") {
    ...
    switch (codeGenType) {
      ...
      case "decl": {
        ...
        codeGen.add(
          `${importingHelper.asReferencedName(responseType)} ${camelCase(name)}(HxContext ctx);`
        );
        codeGen.add("");
        return;
      }
  ...
  if (adlType) {
    if (adlType.decl.type_.kind === "type_" || adlType.decl.type_.kind === "newtype_") {
      ...
      addCode(importingHelper, loadedAdl, codeGenType, codeGen, adlType.decl.type_.value.typeExpr, name, comment, apiDefaults);
    }
    return;
  }
...
async function generateTypescriptService(params: {
  ...
  const { adlSearchPath, outfile, javapackage, apimodule, apiname, adlFiles, serviceclass } = params;
  ...
  importingHelper.addType(apiReqsTypeExpr);

  // start rendering code:
  const code = new CodeGen();
  code.add("/** Some java file comment */");
  ...
  for (const apiEntry of apiRequestsStruct.fields) {
    addCode(importingHelper, loadedAdl, "collect", code, apiEntry.typeExpr, apiEntry.name, getComment(apiEntry), apiEntry.default);
  }
  ...
  importingHelper.modulesImports.forEach((imports_: Set<string>, module: string) => {
    ...
    modImports.forEach(m => {
      code.add(`import ${jp !== null ? jp : module}.${m};`);
    })
  });
  ...
  code.add("");
  code.add(`  static final public ${apiname} REQUESTS = new ${apiname}();`)
  code.add("}");
  ...
  for (const apiEntry of apiRequestsStruct.fields) {
    addCode(importingHelper, loadedAdl, "decl", classBody, apiEntry.typeExpr, apiEntry.name, getComment(apiEntry), apiEntry.default);
  }

  // generate constructor
  classBody.add("public static void register() {");
  ...
  for (const apiEntry of apiRequestsStruct.fields) {
    addCode(importingHelper, loadedAdl, "ctor", ctorBody, apiEntry.typeExpr, apiEntry.name, getComment(apiEntry), apiEntry.default);
  }
  classBody.add("}");
