...
import { Annotations, Decl, Field, makeScopedDecl, ScopedDecl, ScopedName, TypeExpr } from "../adl-gen/runtime/sys/adlast";
...
type CodeGenType = "decl" | "ctor" | "impl";
...
async function generateTypescriptService(params: {
  ...
  const addType = (typeExpr: TypeExpr) => {
    ...
    if (adlType) {
      if (adlType.decl.type_.kind === "type_" || adlType.decl.type_.kind === "newtype_") {
        addType(adlType.decl.type_.value.typeExpr)
      }
      return
    }
    console.warn(`typescript-services: unrecognized field ${typeExpr.typeRef.value.name} in ${apistructSn}`)
  }

  const addCode = (codeGenType: CodeGenType, codeGen: CodeGen, typeExpr: TypeExpr, name: string, comment: string | null) => {
    if (typeExpr.typeRef.kind !== "reference") {
    ...
    if (adlType) {
      if (adlType.decl.type_.kind === "type_" || adlType.decl.type_.kind === "newtype_") {
        addCode(codeGenType, codeGen, adlType.decl.type_.value.typeExpr, name, comment);
      }
      return
    }
  }

  {
    ...
    for (const apiEntry of apiRequestsStruct.fields) {
      addCode("decl", classBody, apiEntry.typeExpr, apiEntry.name, getComment(apiEntry))
    }
    ...
    for (const apiEntry of apiRequestsStruct.fields) {
      addCode("ctor", ctorBody, apiEntry.typeExpr, apiEntry.name, getComment(apiEntry))
    }
    classBody.add("}");
    ...
    //    }
    for (const apiEntry of apiRequestsStruct.fields) {
      addCode("impl", classBody, apiEntry.typeExpr, apiEntry.name, getComment(apiEntry))
    }
    code.add("");
